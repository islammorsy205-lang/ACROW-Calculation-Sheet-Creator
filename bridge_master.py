...
def generate_multi_case_report(cases_data, proj_info):
    ...
    return out
# ==============================================================================
# 6. MAIN STREAMLIT UI (Descriptive Inputs, 3-Zone Sweeps & The Beast)
# ==============================================================================
    ...
    if "DXF" in mode:
        ...
        if 'bridge_cases' in st.session_state:
            ...
            for c_idx, tab in enumerate(tabs):
                ...
                with tab:
                    ...
                    with c_view:
                        ...
                        # 🎯 الزر السحري (توليد الدياجرامات واستخراج نتائج التشيكات الكلية بـ 3 مناطق للسولجر)
                        if st.button(f"🚀 Generate Detail Diagrams for {case['title']}", type="primary", use_container_width=True, key=f"btn_run_{c_idx}"):
                            with st.spinner(f"Solving Matrix, Auto-Sizing Struts & Extracting 3-Zone Global Checks..."):
                                U, R, net_load = solve_fea_engine(p_nodes, p_elems, p_nloads, p_supps)
                                ...
                                # 🎯 3. الاختراع الأكبر: التفنيط المكاني للسولجر لـ 3 مناطق!
                                max_m, max_v = 0.0, 0.0
                                soldier_mall, soldier_qall = 13.1, 100.8 
                                
                                soldier_zones = {
                                    'Cantilever': {'m': 0.0, 'v': 0.0},
                                    'Web': {'m': 0.0, 'v': 0.0},
                                    'Bottom Slab': {'m': 0.0, 'v': 0.0}
                                }
                                
                                for i_seg, sec in enumerate(case['sec_overrides']):
                                    seg_name = case['segments'][i_seg]['name']
                                    
                                    # تحديد المنطقة بناءً على اسم السكشن المستخرج من الـ DXF
                                    zone_key = 'Bottom Slab' # افتراضي
                                    if seg_name in ['S30', 'S31']: zone_key = 'Bottom Slab'
                                    elif seg_name in ['S40', 'S41']: zone_key = 'Web'
                                    elif 'F' in seg_name: zone_key = 'Cantilever'
                                ...
                                for el in p_elems:
                                    if el['type'] == 'truss':
                                        max_n = max(max_n, np.max(np.abs(el.get('internal', {}).get('N', [0]))))
                                        st_sec = el.get('sec', '')
                                        try:
                                            if st_sec in STRUTS_DB: strut_allow = STRUTS_DB[st_sec]['allow']
                                        except Exception:
                                            pass
            ...
            if st.button("📥 Download Multi-Case Word Report", type="primary", use_container_width=True):
                ...
                st.download_button("💾 Save Full Report", data=doc_out.getvalue(), file_name="Acrow_Bridge_Report.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
...
if __name__ == "__main__":
    render_bridge_module({
        "proj_name": "Acrow Bridges Formwork", 
